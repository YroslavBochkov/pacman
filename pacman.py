...
class FrightenedGhost(Ghost):
    def __init__(self, in_surface, x, y, in_size: int, in_game_controller, sprite_path="images/ghost_fright.png"):
        super().__init__(in_surface, x, y, in_size, in_game_controller, sprite_path)

    def automatic_move(self, in_direction: Direction):
        player_position = translate_screen_to_maze(self._renderer.get_hero_position())
        current_maze_coord = translate_screen_to_maze(self.get_position())

        # Calculate the direction to move away from the player
        diff_x = player_position[0] - current_maze_coord[0]
        diff_y = player_position[1] - current_maze_coord[1]

        if diff_x == 0:
            new_direction = Direction.DOWN if diff_y > 0 else Direction.UP
        elif diff_y == 0:
            new_direction = Direction.LEFT if diff_x < 0 else Direction.RIGHT
        else:
            # Choose a random direction to move away from the player
            directions = [Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT]
            directions.remove(in_direction)
            new_direction = random.choice(directions)

        collision_result = self.check_collision_in_direction(new_direction)
        if not collision_result[0]:
            self.current_direction = new_direction
            self.set_position(collision_result[1][0], collision_result[1][1])
